...
class PromptTrainDataset(Dataset):
    def __init__(self, args):
        ...
        self.de_dict = {'denoise_15': 0, 'denoise_25': 1, 'denoise_50': 2, 'derain': 3, 'dehaze': 4, 'deblur' : 5, 'allweather': 6}
    ...
    def _init_ids(self):
        ...
        if 'dehaze' in self.de_type:
            self._init_hazy_ids()
        if 'allweather' in self.de_type:
            self._init_allweather_ids()

        random.shuffle(self.de_type)

    def _init_allweather_ids(self):
        # 读取用户生成的 allweather_train.txt
        ref_file = os.path.join(self.args.data_file_dir, "allweather_train.txt")
        if not os.path.exists(ref_file):
            print(f"Warning: {ref_file} not found. Please generate it first.")
            self.allweather_ids = []
            return
        ...
        for name in temp_ids:
            # 根据文件名关键词自动分配类别
            if 'rain' in name.lower():
                de_type = 3 # derain
            elif 'fog' in name.lower() or 'haze' in name.lower():
                de_type = 4 # dehaze
            elif 'snow' in name.lower():
                de_type = 6 # desnow (new category)
            else:
                de_type = 3 # 默认归为 derain
            
            self.allweather_ids.append({
                "clean_id": name, 
                "de_type": de_type,
                "is_allweather": True
            })
        
        print("Total All-Weather Ids : {}".format(len(self.allweather_ids)))

    def _init_clean_ids(self):
        ref_candidates = [
            os.path.join(self.args.data_file_dir, "noisy", "denoise_airnet.txt"),
            os.path.join(self.args.data_file_dir, "noisy", "denoise.txt"),
        ]
        ref_file = next((p for p in ref_candidates if os.path.exists(p)), None)
        if ref_file is None:
            raise FileNotFoundError(
                "No denoise file list found. Tried: " + ", ".join(ref_candidates)
            )
        temp_ids = []
        temp_ids+= [id_.strip() for id_ in open(ref_file)]
        clean_ids = []
    ...
    def _merge_ids(self):
        ...
        if "allweather" in self.de_type:
            self.sample_ids += self.allweather_ids
    ...
    def __getitem__(self, idx):
        ...
        if sample.get("is_allweather"):
            # All-Weather 数据集处理逻辑：lq 和 gt 路径对应，文件名一致
            lq_path = os.path.join(self.args.allweather_lq, sample["clean_id"])
            gt_path = os.path.join(self.args.allweather_gt, sample["clean_id"])
            
            degrad_img = crop_img(np.array(Image.open(lq_path).convert('RGB')), base=16)
